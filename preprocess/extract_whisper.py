...
def get_mapped_whisper_features(dataset, dataset_type, raw_whisper_features):
    ...
    with open(os.path.join(MCEP_dir, "{}.pkl".format(dataset_type)), "rb", encoding="utf-8") as f:
        mceps = pickle.load(f)
...
def extract_whisper_features(dataset, dataset_type, batch_size=30):
    print("-" * 20)
    ...
    with open(os.path.join(data_dir, "{}.json".format(dataset_type)), "r", encoding="utf-8") as f:
    ...
    with open(os.path.join(output_dir, "{}.pkl".format(dataset_type)), "wb", encoding="utf-8") as f:
