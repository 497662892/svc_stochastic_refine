...
from cuhkszsvc.utils.dsp import compress, audio_to_label
...
def extract_label_features_of_dataset(
    output_path,
    dataset_path,
    dataset,
    fs,
    ...
    output_dir = os.path.join(output_path, dataset, "label/{}".format(fs))
    types = ["train", "test"]
    for dataset_type in types:
        ...
        if is_mu_law:
            ...
            audio_features = [compress(audio, bits) for audio in audio_features]

        # Extract label features
        print("\nExtracting label features...")
        label_features = [audio_to_label(audio, bits) for audio in audio_features]
