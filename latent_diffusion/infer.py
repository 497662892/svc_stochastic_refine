...
def save_pred_audios(
    pred,mcep_real, args, index, uids, output_dir, target_base_f0, upaths = None, ratio = 0.5):
    ...
    pred = pred.cpu()
    frame_len = min(len(f0), len(pred))
    
    f0 = f0[:frame_len]
    sp = sp[:frame_len]
    ap = ap[:frame_len]
    mcep = pred[:frame_len]
    mcep_real = mcep_real[:frame_len]

    criterion = nn.MSELoss(reduction='none')
    loss = criterion(mcep,mcep_real)
    loss = torch.sum(loss) / frame_len
    
    print("the MSE loss is:", loss)
    
    criterion2 = nn.L1Loss(reduction='none')
    loss2 = criterion2(mcep,mcep_real)
    loss2 = torch.sum(loss2) / frame_len
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--checkpoint',
        type=str,
        default='logs/diffsvc_MCEP_x0/checkpoints/best.ckpt',
        help='path to the checkpoint model'
    )
    parser.add_argument(
        '--config',
        type=str,
        default='configs/infer/diffsvc_MCEP_x0.yaml',
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help='ratio of the lowest pitch in an utterance'
    )
...
with torch.no_grad():
    with model.ema_scope():
        for i, cases in enumerate(tqdm(test_dataloader)):
            ...
            first_stage_pre = torch.unsqueeze(cases["first_stage"],dim=1).to(device)
            ...
            #sampling
            samples_ddim, _ = model.sample_log(cond=c, batch_size=shape[0],ddim = opt.ddim, ddim_steps=opt.steps)
            
            x_samples_ddim = model.decode_first_stage(samples_ddim)
            #get the predicted mcep
            pred_mcep = torch.mean(x_samples_ddim[0],dim = 0) #take the first channel

            save_pred_audios(pred_mcep, mecp.to("cpu"), opt, index=i, uids=uids,upaths = upaths, output_dir=opt.outdir,target_base_f0=target_base_f0)
