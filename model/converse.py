...
from config import dataset2wavpath, MCEP_DIM, WORLD_SAMPLE_RATE
import extract_sp
import extract_mcep
...
def get_pred_audios(
    ...
    for i, uid in enumerate(tqdm(uids)):
        ...
        ap = ap[:frame_len]
        # get real_MCEP
        real_mcep = extract_mcep.SP2mgc(sp, mcsize=MCEP_DIM - 1, fs= WORLD_SAMPLE_RATE)
        # Get sp_pred
        ...
        #mse loss
        loss = np.sum(np.square(real_mcep - mcep))/frame_len
        print("the mse loss of mcep is:", loss)
        
        #l1 loss
        loss2 = np.sum(np.abs(real_mcep - mcep))/frame_len
        print("the l1 loss of mcep is:", loss2)
